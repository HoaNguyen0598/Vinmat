  ...
  const getTimeDay = () => {
   ...
   useEffect(() => {
    if(dataShelf && dataShelf2 && !isCheckDropdown){
      ...
      //count shelf1
      const item1 = dataShelf.map(item =>  parseInt(item.number_count)) // get count data shelf 1
                    .reduce((accumulator, currentValue) => accumulator + currentValue)

      const item2 = dataShelf2.map(item =>  parseInt(item.number_count)) // get count data shelf 2
                    .reduce((accumulator, currentValue) => accumulator + currentValue)

      let total = item1 + item2;
      setTotalShelfUse(total)

      // tính tủ đang giữ đồ và tủ đang rỗng dropdown 1
      ...
      if(dataShelf3){
            const item = dataShelf3.map(item =>  parseInt(item.number_count)) // get count data shelf 1
                                   .reduce((accumulator, currentValue) => accumulator + currentValue)
